...
const AppProvider = ({ children }) => {
  ...
  const changePagePending = (pendingPage) => {
    dispatch({
  ...
  const getPending = async () => {
    ...
    let url = `/customer/getPending?page=${pendingPage}`;
    dispatch({type: GET_PENDING_BEGIN})

    try {
      const {data} = await authFetch(url);
      const {pendingData, totalPending, numOfPending} = data
      dispatch({
  ...
  const getListBrand = async () => {
    dispatch({type: GET_LISTBRAND_BEGIN})
    
    try {
      const {data} = await authFetch('/customer/brand');
      const {listOfBrand, listOfPart} = data
      dispatch({
        type: GET_LISTBRAND_SUCCESS,
        payload: {
          listOfBrand,
          listOfPart
        }
