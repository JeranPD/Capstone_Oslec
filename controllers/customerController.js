...
const getBrand = async (req, res) =>{
  ...
  const brands = allRecords.map((brand) => {
    return [brand.brand]
     
  })
  const flattened = brands.flatMap(brand => brand);
  
  const result = flattened.map((brand) =>{
    return brand.trim();
  })
  const listOfBrand = [...new Set(result)];

  const parts = allRecords.map((part) => {
    return [part.replacedParts]
     
  })
  const flattenedPart = parts.flatMap(part => part);
  
  // const resultPart = flattenedPart.map((part) =>{
  //   return part.trim();
  // })
  // console.log(resultPart)
  const listOfPart = [...new Set(flattenedPart)];

  
  res.status(StatusCodes.OK).json({ listOfBrand, listOfPart });
}
...
const getPending = async (req, res) => {
  ...
  const pendingData = await allRecords;
  

  const totalPending = await AddCustomer.countDocuments(queryObject);
  const numOfPending = Math.ceil(totalPending / limit);

  res.status(StatusCodes.OK).json({pendingData, totalPending, numOfPending});
};
export {
  addNewCustomer,
  getAllCustomer,
